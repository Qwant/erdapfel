...
function convertTask(buffer, options) {
  ...
  const manageContext = options.manageContext || false

  const returnObj = poStrToObject(buffer, manageContext)
 ...
 *
 * @param {String} buffer the contents of a po file.
 * @returns {Object} The translations in name-value pairs.
 */
function poStrToObject(buffer, target, manageContext) {
  // Prepare output
  var options = {};
  var entries = {};
